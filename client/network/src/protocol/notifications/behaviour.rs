...
impl Notifications {
	...
	fn protocol_report_accept(&mut self, index: sc_peerset::IncomingIndex) {
		let incoming = if let Some(pos) = self.incoming.iter().position(|i| i.incoming_id == index)
		{
			self.incoming.remove(pos)
		} else {
			error!(target: LOG_TARGET, "Protocol => Accept({:?}): Invalid index", index);
			return
		};
		...
		if !incoming.alive {
			trace!(
				target: LOG_TARGET,
				"Protocol => Accept({:?}, {}, {:?}): Obsolete incoming",
				index,
				incoming.peer_id,
				incoming.set_id
			);
		...
		match mem::replace(state, PeerState::Poisoned) {
			// Incoming => Enabled
			PeerState::Incoming { mut connections, incoming_index, accepted, .. } => {
				if !accepted {
					log::error!(
						target: LOG_TARGET,
	...
	/// Protocol rejected inbound substream.
	fn protocol_report_reject(&mut self, index: sc_peerset::IncomingIndex) {
...
mod tests {
	...
	#[tokio::test]
	async fn connection_and_substream_open() {
		let (mut notif, _peerset, _notif_service) = development_notifs();
		let peer = PeerId::random();
		let conn = ConnectionId::new(0usize);
		let set_id = sc_peerset::SetId::from(0);
	...
	fn peerset_report_connect_with_incoming_peer() {
		let (mut notif, _peerset, _) = development_notifs();
		let peer = PeerId::random();
		let set_id = sc_peerset::SetId::from(0);
		let conn = ConnectionId::new(0usize);
		let connected = ConnectedPoint::Listener {
